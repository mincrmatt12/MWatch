...
def decode_compressed(width, height, wordlen, datao):
    ...
    while len(bits) < width*height:
        ...
        if sf and cmd_1 == [0, 0]:
            ...
            return output, disasm[:-1], insn_size
        elif cmd_1 == [0, 0]:
            # duplicate command
            cmd_2 = readbit()
            if cmd_2 == 1:
    ...
    return output, disasm, insn_size
...
with open(sys.argv[1], "rb") as f:
    ...
    for j, i in enumerate(range(dataptr, dataptr+512, 2)):
        ...
        if not compressed:
            ...
            data, log, compressed_size = decode_compressed(wid, hgt, third, datablb[offset+6:])
            ...
            uncomp_width = wid if wid % 8 == 0 else wid + (8 - (wid % 8))
            uncompressed_size = uncomp_width * hgt
            print(f" Ratio: {compressed_size}/{uncompressed_size} = {(compressed_size/uncompressed_size)*100:2.02f}%")
